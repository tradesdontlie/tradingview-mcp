...
export async function launch({ port, kill_existing, deps: _deps } = {}) {
  ...
  if (!tvPath && platform === "darwin") {
    try {
      const found = deps
        .execSync('mdfind "kMDItemFSName == TradingView.app" | head -1', {
          timeout: 5000,
        })
        .toString()
        .trim();
  ...
  if (!tvPath) {
    throw new Error(
      `TradingView not found on ${platform}. Searched: ${candidates.join(", ")}. Launch manually with: /path/to/TradingView --remote-debugging-port=${cdpPort} (note: TradingView v2.14.0+ may reject this flag)`,
    );
  }
    ...
    if (platform === "darwin") {
      ...
      if (appMatch) {
        ...
        try {
          ...
          /* ignore — open may return non-zero even on success */
        }
      } else {
        // No .app bundle found; try spawning without the flag as last resort.
        const fallback = deps.spawn(tvPath, [], {
          detached: true,
          stdio: "ignore",
        });
        fallback.unref();
  ...
  if (spawnFailed) {
    return {
      ...
      cdp_ready: false,
      error:
        `TradingView launched but CDP not available on port ${cdpPort}. ` +
        "This is likely TradingView v2.14.0+ (Electron 38 / Node 22) which rejects --remote-debugging-port as a CLI flag. " +
        "Workaround: pkill -f TradingView; sleep 2; open -a TradingView --args --remote-debugging-port=" +
        cdpPort,
    };
  }
