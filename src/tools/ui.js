...
export function registerUiTools(server) {
  ...
  }, async ({ by, value, target_id }) => {
    try { return jsonResult(await withTarget(target_id, () => core.click({ by, value }))); }
  ...
  server.tool('ui_open_panel', 'Open, close, or toggle TradingView panels (pine-editor, strategy-tester, watchlist, alerts, trading)', {
    ...
    target_id: targetIdParam,
  }, async ({ panel, action, target_id }) => {
    try { return jsonResult(await withTarget(target_id, () => core.openPanel({ panel, action }))); }
    catch (err) { return jsonResult({ success: false, error: err.message }, true); }
  });
  ...
  server.tool('ui_fullscreen', 'Toggle TradingView fullscreen mode', {
    ...
    try { return jsonResult(await withTarget(target_id, () => core.fullscreen())); }
    catch (err) { return jsonResult({ success: false, error: err.message }, true); }
  });
  ...
  server.tool('layout_list', 'List saved chart layouts', {
    ...
    try { return jsonResult(await withTarget(target_id, () => core.layoutList())); }
    catch (err) { return jsonResult({ success: false, error: err.message }, true); }
  });
  ...
  server.tool('layout_switch', 'Switch to a saved chart layout by name or ID', {
    ...
    try { return jsonResult(await withTarget(target_id, () => core.layoutSwitch({ name }))); }
    catch (err) { return jsonResult({ success: false, error: err.message }, true); }
  });
  ...
  server.tool('ui_keyboard', 'Press keyboard keys or shortcuts (e.g., Enter, Escape, Alt+S, Ctrl+Z)', {
    ...
    target_id: targetIdParam,
  }, async ({ key, modifiers, target_id }) => {
    try { return jsonResult(await withTarget(target_id, () => core.keyboard({ key, modifiers }))); }
    catch (err) { return jsonResult({ success: false, error: err.message }, true); }
  });
  ...
  server.tool('ui_type_text', 'Type text into the currently focused input/textarea element', {
    ...
    target_id: targetIdParam,
  }, async ({ text, target_id }) => {
    try { return jsonResult(await withTarget(target_id, () => core.typeText({ text }))); }
    catch (err) { return jsonResult({ success: false, error: err.message }, true); }
  });
  ...
  server.tool('ui_hover', 'Hover over a UI element by aria-label, data-name, or text content', {
    ...
    try { return jsonResult(await withTarget(target_id, () => core.hover({ by, value }))); }
    catch (err) { return jsonResult({ success: false, error: err.message }, true); }
  });
  ...
  server.tool('ui_scroll', 'Scroll the chart or page up/down/left/right', {
    ...
    target_id: targetIdParam,
  }, async ({ direction, amount, target_id }) => {
    try { return jsonResult(await withTarget(target_id, () => core.scroll({ direction, amount }))); }
    catch (err) { return jsonResult({ success: false, error: err.message }, true); }
  });
  ...
  server.tool('ui_mouse_click', 'Click at specific x,y coordinates on the TradingView window', {
    ...
    target_id: targetIdParam,
  }, async ({ x, y, button, double_click, target_id }) => {
    try { return jsonResult(await withTarget(target_id, () => core.mouseClick({ x, y, button, double_click }))); }
    catch (err) { return jsonResult({ success: false, error: err.message }, true); }
  });
  ...
  server.tool('ui_find_element', 'Find UI elements by text, aria-label, or CSS selector and return their positions', {
    ...
    target_id: targetIdParam,
  }, async ({ query, strategy, target_id }) => {
    try { return jsonResult(await withTarget(target_id, () => core.findElement({ query, strategy }))); }
    catch (err) { return jsonResult({ success: false, error: err.message }, true); }
  });
  ...
  server.tool('ui_evaluate', 'Execute JavaScript code in the TradingView page context for advanced automation', {
    ...
    try { return jsonResult(await withTarget(target_id, () => core.uiEvaluate({ expression }))); }
    catch (err) { return jsonResult({ success: false, error: err.message }, true); }
  });
}
