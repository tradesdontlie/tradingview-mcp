...
- **T123 — 8dp price rounding** (upstream `a184e66`/#77). `getOhlcv` summary + `getPineLines`/`Labels`/`Boxes` rounded prices to 2dp, flattening sub-cent/fractional levels. Replaced the 6 **price** sites with a shared `roundPrice()` at 8dp (percentages/volumes untouched). LIVE on the ASTA chart: 3Cs/Patterns levels read exact — `29.8755`, `27.5454`, `55.885` — where 2dp would have flattened them.
